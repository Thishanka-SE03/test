...
import { OrbitControls, Environment, ContactShadows, Float, MeshWobbleMaterial } from '@react-three/drei';
  ...
  <group position={position}>
    <Float speed={2} rotationIntensity={0.5} floatIntensity={0.5}>
      <mesh>
        ...
        <MeshWobbleMaterial factor={0.4} speed={1} color="#064e3b" roughness={0.8} />
      </mesh>
...
const RainParticles = () => {
  ...
  return (
    <group ref={rainRef}>
      {[...Array(20)].map((_, i) => (
        <mesh key={i} position={[(Math.random() - 0.5) * 2, Math.random() * 3, (Math.random() - 0.5) * 2]}>
    ...
    <group ref={treeRef} position={[0, -2, 0]}>
      ...
      {stage === 5 && (
        <group>
          ...
          <Branch position={[0, 1.3, 0]} rotation={[0, 2.5, -0.8]} scale={0.8} />
      ...
      {stage === 6 && (
        <group>
          ...
          {[0, 2.1, 4.2].map((rot, i) => (
            <group key={i} rotation={[0, rot, 0.7]} position={[0, 1.2 + i * 0.4, 0]}>
              <Branch scale={0.9} />
            </group>
          ))}
      ...
      {stage === 7 && (
        <group>
          ...
          {[0, 1.5, 3, 4.5].map((rot, i) => (
            <group key={i} rotation={[0, rot, 0.6]} position={[0, 1.2 + i * 0.5, 0]}>
              <Branch scale={1} />
            </group>
          ))}
      ...
      {stage === 8 && (
        <group>
          ...
          {[0, 1.2, 2.4, 3.6, 4.8].map((rot, i) => (
            <group key={i} rotation={[0, rot, 0.5]} position={[0, 1.2 + i * 0.4, 0]}>
...
export default function EcoApp() {
  ...
  return (
    <div className="page-container">
      <Styles />
      <div className="molten-bg"><div className="molten-orb" /></div>

      <div className="app-wrapper">
        <header className="nav-header">
          <div className="logo-text">
            <img src='src/assets/images/logo.png' style={{ width: '40px', height: '40px' }} alt="logo" />
            EcoSphere
          </div>
        </header>

        <main className="dashboard-grid">
          {/* Left: Test Controller */}
          <section className="glass-panel">
            <h2 style={{ marginBottom: '10px', fontSize: '1.2rem' }}>Live Growth Sync</h2>
            <p style={{ fontSize: '0.8rem', opacity: 0.7, marginBottom: '20px' }}>
              Slide to preview 8-stage evolution (Simulating DB values):
            </p>
            
            <input 
                type="range" min="0" max="100" value={points} 
            ...
            <div style={{ marginTop: '20px', padding: '15px', background: 'rgba(255,255,255,0.05)', borderRadius: '15px' }}>
                <h4 style={{ margin: '0 0 5px 0', fontSize: '0.8rem', color: '#34d399', textTransform: 'uppercase' }}>Current Phase</h4>
                <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{STAGE_NAMES[currentStageIndex]}</div>
                <div style={{ fontSize: '0.9rem', opacity: 0.6 }}>{points} Eco Points</div>
            </div>
          </section>
          ...
          <section className="canvas-section">
            <div className="points-pill">
              <h1 style={{ margin: 0, fontSize: '1.4rem', letterSpacing: '2px', textTransform: 'uppercase' }}>
                {STAGE_NAMES[currentStageIndex]}
              </h1>
              <div className="progress-track">
                <motion.div className="progress-fill" animate={{ width: `${points}%` }} transition={{ type: 'spring', stiffness: 50 }} />
              </div>
            </div>
            ...
            <div className="canvas-holder">
              <Canvas shadows dpr={[1, 2]}>
                <Suspense fallback={null}>
                  ...
                  <ContactShadows opacity={0.4} scale={15} blur={2.5} far={10} color="#000" />
          ...
          <section className="glass-panel">
            <h3 style={{ marginBottom: '20px', fontSize: '1rem' }}>Environmental Impact</h3>
            <div style={{ display: 'grid', gap: '20px' }}>
                <ImpactItem icon={<Droplets size={20} color="#34d399" />} label="Water Purified" value={`${points * 5}L`} />
                <ImpactItem icon={<Leaf size={20} color="#34d399" />} label="CO₂ Offset" value={`${(points * 0.4).toFixed(1)}kg`} />
                <ImpactItem icon={<ShieldCheck size={20} color="#34d399" />} label="Growth Rank" value={`Lvl ${currentStageIndex + 1}`} />
            </div>
          </section>
        </main>
      </div>
...
const ImpactItem = ({ icon, label, value }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
        <div style={{ padding: '10px', background: 'rgba(52, 211, 153, 0.1)', borderRadius: '12px' }}>{icon}</div>
        <div>
            <div style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>{value}</div>
            <small style={{ opacity: 0.5, textTransform: 'uppercase', fontSize: '0.7rem' }}>{label}</small>
        </div>
    </div>
);